...
def mess_up_choices(row: Row):
    ...
    for idx, i in enumerate(correct_list):
        ...
        temp[j] = choices[i]
        correct_list[idx] = j
    answer = "".join(map(str, correct_list)).translate(
...
def create(xlsx_path, number_of_copies, is_mess_up):
    ...
    for count in range(number_of_copies):
        ...
        document.styles["Normal"].font.name = "仿宋"
        ...
        document.add_paragraph(dan_para)  # 考题
        ...
        document.add_paragraph(duo_para)
        ...
        document.add_paragraph(tian_para)
        answerdocument.add_paragraph(tian_para)  # 答案

        add_sheet_part(tian_sheet, document,
                       answerdocument, 8, is_choice=False)
        # ------------------------------*************************------------------------------
        pan_para = "第四部分  判断题(8题)"
        document.add_paragraph(pan_para)
        answerdocument.add_paragraph(pan_para)  # 答案

        add_sheet_part(pan_sheet, document, answerdocument, 8, is_choice=False)
        # ------------------------------*************************------------------------------
        jian_para = "第五部分  简答题(2题)"
        document.add_paragraph(jian_para)
        answerdocument.add_paragraph(jian_para)  # 答案

        add_sheet_part(jian_sheet, document,
                       answerdocument, 2, is_choice=False)
        # ------------------------------*************************------------------------------

        # 保存文件
        answer_savepath = os.path.join(output_path, f"测试{count}答案.docx")
